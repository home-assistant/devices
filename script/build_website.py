...
class DeviceSite:
    ...
    @property
    def title(self) -> str:
        return f'{self.info["manufacturer_name"]} {self.info["model_name"]}'
...
def build():
    ...
    for integration in sorted(
        ...
        integration_devices = build_integration(integration)
        index_json.append(integration.name)
        title = INTEGRATIONS_INFO[integration.name]["title"]
        index_markdown.append(f"- [{title}]({WEBSITE_BASE_PATH}{integration.name}/) ({len(integration_devices)} devices)")
